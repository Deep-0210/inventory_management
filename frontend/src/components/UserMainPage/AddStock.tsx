...
const AddStock = () => {
    ...
    const [data, setData] = useState<EditStockData>({ '_id': "", "productName": "", "productPrice": "", "productQuantity": "", update: false, delete: false })
    // useEffect for set the edit data
    useEffect(() => {
        if (data?.productName !== "" && data?.update) {
            setValues(data)
    ...
    const updateStockData = (value: UserStock) => {
        ...
        putRequest('updateStockData', JSON.stringify(stockData)).then((res) => {
            resetForm()
            setReApiCall(1)
            setData({ '_id': "", "productName": "", "productPrice": "", "productQuantity": "", delete: false, update: false })
            setTimeout(() => {
                setReApiCall(0)
            }, 500)
                ...
                <div className='w-[320px] border border-white rounded-lg mx-auto p-5 my-10'>
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className='mt-5 w-max mx-auto'>
                            <Button placeholder={'addStock'} color="green" type='submit'>{`${data.productName !== "" && data?.update ? 'Update Stock' : '+ Add Stock'}`}</Button>
                        </div>
                    </form>
                </div>
